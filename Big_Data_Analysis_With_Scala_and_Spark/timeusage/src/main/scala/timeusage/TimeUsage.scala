...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType = {
    val schemaNames = columnNames.tail.map(x=> StructField(x,DoubleType,nullable = false))
    StructType(StructField(columnNames(0),StringType,nullable = false)::schemaNames)
  }
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    var cpr: List[Column] = List()
    var cwk: List[Column] = List()
    var cor: List[Column] = List()

    columnNames.foreach{ name =>
      if (primary.exists(prefix => name.startsWith(prefix))) {
        cpr = new Column(name)::cpr
      }
      else if (working.exists(prefix => name.startsWith(prefix))) {
        cwk = new Column(name)::cwk
      }
      else if (other.exists(prefix => name.startsWith(prefix))) {
        cor = new Column(name)::cor
      }
    }
    (cpr,cwk,cor)
  }
  ...
  def timeUsageSummary(
    ...
    val ageProjection: Column =
      when($"teage"<=22 && $"teage">15,"young")
        .when($"teage"<=55 && $"teage">23, "active")
        .otherwise("elder")
        .as("age")
        .cast(StringType)
  ...
  def timeUsageSummaryTyped(timeUsageSummaryDf: DataFrame): Dataset[TimeUsageRow] =
    {
      timeUsageSummaryDf.map(row => {TimeUsageRow(
          row.getAs[String]("working"), row.getAs[String]("sex"),
          row.getAs[String]("age"), row.getAs[Double]("primaryNeeds"),
          row.getAs[Double]("work"), row.getAs[Double]("other")
        )})
    }
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    summed.groupByKey(l=>(l.working,l.sex,l.age)).agg(
      ...
      .map(l=>TimeUsageRow(l._1._1,l._1._2,l._1._3,l._2,l._3,l._4))
      .orderBy($"working", $"sex", $"age")
  }
}
