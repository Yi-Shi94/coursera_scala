...
class Simulator(val taskSupport: TaskSupport, val timeStats: TimeStatistics) {
  ...
  def updateBoundaries(boundaries: Boundaries, body: Body): Boundaries = {
    val nb = new Boundaries
    nb.minX = Math.min(boundaries.minX,body.x)
    nb.maxX = Math.max(boundaries.maxX,body.x)
    nb.minY = Math.min(boundaries.minY,body.y)
    nb.maxY = Math.max(boundaries.maxY,body.y)
    nb
  }

  def mergeBoundaries(a: Boundaries, b: Boundaries): Boundaries = {
    val nb = new Boundaries
    nb.minX = Math.min(a.minX,b.minX)
    nb.maxX = Math.max(a.maxX,b.maxX)
    nb.minY = Math.min(a.minY,b.minY)
    nb.maxY = Math.max(a.maxY,b.maxY)
    nb
  }
  ...
  def computeSectorMatrix(bodies: Seq[Body], boundaries: Boundaries): SectorMatrix = timeStats.timed("matrix") {
    val parBodies = bodies.par
    parBodies.tasksupport = taskSupport
    parBodies.aggregate(new SectorMatrix(boundaries,SECTOR_PRECISION))({case(sectorMatrix, body)=>sectorMatrix.+=(body)}, _.combine(_))
  }
