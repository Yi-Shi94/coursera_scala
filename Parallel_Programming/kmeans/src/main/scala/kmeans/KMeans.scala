...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    if (points.isEmpty) means.par.map(x=>(x,Nil)).toMap

    else {
      val pointMap = points.par.groupBy(y=>findClosest(y,means))
      means.par.map(i=>i->pointMap(i)).toMap
    }
  }
  ...
  def update(classified: GenMap[Point, GenSeq[Point]], oldMeans: GenSeq[Point]): GenSeq[Point] = {
    oldMeans.par.map(i=> findAverage(i,classified(i)))
  }
